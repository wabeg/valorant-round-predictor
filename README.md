...
## Executive Summary
Valorant is one of the fastest growing Esports in the world. Since it is still new there has yet to be alot of round win predictors in circulation. In a simmilar game called CSGO, which has been out for 10 years, round win probabilty is shown as a live stat on pro match streams. This project attempts to detemine the viability of a simmilar model in Valorant. After using a scraper for vlr.gg supplied to me by fellow data scientist Joshua Broas I collected up with data on around 16,000 of the most recent professional matches. 

Through EDA I found that different rounds had different trends for each varaible, as a result I split the dataset into 6 smaller datasets for Round 1, Round 13, Round 2, Round 3, Overtime Rounds, and the rest of my data. I then attempted to find the best classification models for each of these dataset and extract feature importances. I found that my dataset had correlation between columns so I also used PCA to decorrelate my data. In most cases the models did not improve from PCA and the loss of information from losing feature extraction led me to using my original dataset in the final models. Every one of the final models that I created beat their respective baseline scores. A summary of the findings can be found below:
...
- Pistol round winner was by far the most important feature in this model
...
- Almost all of the agents had simmilar feature importances.
...
The initial dataset was aquired from Kaggle. It only contained usable data on 10,000 matches that were all over a year old. 

## Data Acquisition: VLR.gg webscraper ([GitHub](https://github.com/jbroas/valorant-db))
After not being satisfied with the publicly available datasets for professional matches, I contacted the creator of the kaggle dataset to inquire if I could use his webscraper to collect more recent data. He kindly shared the git repo with me.

The final dataset contained almost data for almost 20,000 professional games of valorant. 

## Data Ingestion

The webscraper outputted data in the form of a database. I ran a function that collected round by round information as well as team agent compositions for 336,913 rounds of professional valorant from the database.

## Data Cleaning:
### Feature Removal:
- The only features that I chose from the scraped database were used so the only feature that was removed before modeling was 'Map' since there was not enough data to make a seperate model for each map.
...
|------|------|-------------|
| **Team1Win** | boolean | Whether or not Team 1 won the match |
| Team1WinLag1 | boolean | Whether or not Team 1 won the match in the last round |
| Team1WinLag2 | boolean | Whether or not Team 1 won the match in the second to last round |
| Team1WinLag3 | boolean | Whether or not Team 1 won the match in the third to last round |
| Team1WinLag4 | boolean | Whether or not Team 1 won the match in the fourth to last round |
| RoundNo | int | The number of the round in which the match was played |
| RoundDiff | int | The difference in round number between the current match and the last match |
...
| Team1_eco | boolean | Whether or not Team 1 was in eco mode |
| Team1_full-buy | boolean | Team 1 full buy (20,000k+) |
| Team1_semi-buy | boolean | Team 1 was in semi buy mode |
| Team1_semi-eco | boolean | Whether or not Team 1 was in semi eco mode |
| Team2_eco | boolean | Whether or not Team 2 was in eco mode |
| Team2_full-buy | boolean | Whether or not Team 2 full bought |
| Team2_semi-buy | boolean | Whether or not Team 2 was in semi buy mode |
| Team2_semi-eco | boolean | Whether or not Team 2 was in semi eco mode |
...
#### Round 1 Model
Only Gradient Boosting (.563) beat the baseline (.559). **XGBoost (.55)** was still chosen over Gradient boosting since has more balanced precision and recall for 0 and 1 which is better for the purpose of the models.
...
#### Round 4 Model
XGBoost (.68) and **Gradient Boosting(.69)** perform better than the Time Lag 1 baseline (.60). Both models have simmilar balancing between precisions and recall so Gradient Boosting is chosen since it has a higher accuracy score.

#### Overtime Model
**XGBoost (.60)** and Gradient Boosting (.59) have simmilar accuracies and are both above the baseline (.58). XGBoost is chosen since all of its accuracy is higher and its precision and recall are more balanced for 0 and 1.

#### Mid-Game Model
**XGBoost (.67)** performs better than the baseline (.60) and Gradient Boosting (.66) so it is chosen.

## Findings from Models
- The presence of a healer (skye and sage) on the team is the largest predictor a winnning a pistol round
...
## Limitations
- Since my model was already split by round I was unable to split by map as well since the datasets would become to small. Once more games have been played this may be possible to create an even more accurate model. 

## Recommendations / Next Steps
The next steps would be to use software to scrape data directly from maps. This would allow for mid-round prediction to be possible. Another next step would be to create models on the data by map and round. Introducing player and team data could also allow for more accurate predictions. Another way to make the feature importances of the models even better would be to swap all the Team 1 data points with Team 2 and train the models on the combined dataset. This prevent the model from favoring either team over the other when creating the models. 
